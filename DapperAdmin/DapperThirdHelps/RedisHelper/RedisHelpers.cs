...
namespace DapperThirdHelps.RedisHelper
{
    ...
    public class RedisHelpers
    {
        ...
        public RedisHelpers(int dbNum = 0) : this(dbNum, null)
        {
        }

        public RedisHelpers(int dbNum, string readWriteHosts = "")
        {
            DbNum = dbNum;
            _conn =
        ...
        /// 存储缓存List
        /// </summary>
        /// <typeparam name="T">集合对象实体</typeparam>
        /// <param name="key"></param>
        /// <param name="List">集合对象</param>
        /// <param name="expiry">过期时间</param>
        public void ListSet<T>(string key, List<T> List, TimeSpan? expiry = default(TimeSpan?))
        {
            key = AddSysCustomKey(key);
            foreach (var single in List)
            {
                Do(db => db.ListRightPush(key, ConvertJson(single)));
            }
        }
        ...
        {
            key = AddSysCustomKey(key);
            var vList = Do(db => db.ListRange(key));
            List<T> result = new List<T>();
            return ConvetList<T>(vList);
        }
        ...
        /// <param name="List">集合对象</param>
        /// <param name="expiry">过期时间</param>
        public async Task<List<T>> ListSetAsync<T>(string key, List<T> List, TimeSpan? expiry = default(TimeSpan?))
        {
            foreach (var single in List)
            {
                await Do(db => db.ListRightPushAsync(key, ConvertJson(single)));
            }
            return List;
        }
